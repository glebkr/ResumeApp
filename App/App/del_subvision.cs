...
namespace App
{
    public partial class del_subvision : Form
    {
        public del_subvision()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e) //Кнопка удаления данных из таблицы Структурные подразделения по id структурного подразделения
        {
            try
            {
                if (textBox1.Text == "")
                ...
                {
                    string query = $"DELETE FROM subvisions WHERE subvision_title = '{textBox1.Text}'";
